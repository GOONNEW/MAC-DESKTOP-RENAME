...
macOS는 Mission Control의 데스크탑 이름을 바꾸는 공개 API를 제공하지 않습니다. 이 앱은 비공개 SkyLight API로 공간 목록과 현재 공간을 읽고, 화면 위쪽을 주기적으로(0.7초) 캡처해 "데스크탑 N" 라벨 줄이 보이면 그 자리에 작은 패널로 이름표를 그립니다. 최신 macOS에서는 Mission Control이 열려도 앱이 관찰할 수 있는 상태(Dock 접근성 트리, 창 목록, WindowServer 알림, 메뉴 막대)가 변하지 않아서, 화면을 직접 보는 방식만 남았습니다. 화면이 이전과 같으면 글자 인식은 건너뜁니다. 그래서:
